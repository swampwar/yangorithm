package ThinkDataStructure.ch02;
...
public class MyArrayList<T> implements List<T>{
	...
	public boolean add(T element) {
		...
		if(size >= array.length) {
			T[] bigger_array = (T[]) new Object[array.length * 2];
			System.arraycopy(array, 0, bigger_array, 0, array.length);
			array = bigger_array;
		}
	...
	public T set(int index, T element) {
		// TODO: FILL THIS IN!
		// 해당 index의 요소를 element로 교체하고 이전 요소를 반환한다.
		if(index < 0 || index >= size) {
			throw new IndexOutOfBoundsException();
		}
		
		T before_element = array[index];
		array[index] = element;
